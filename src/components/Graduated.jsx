              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {/* Progress Tracking Display - from progressHistory */}
                    {startup.progressHistory && startup.progressHistory.length > 0 && (() => {
                      // Get the latest progress entry
                      const latestProgress = startup.progressHistory[startup.progressHistory.length - 1];
                      let notes = {};
                      try {
                        notes = typeof latestProgress.notes === 'string' ? JSON.parse(latestProgress.notes) : (latestProgress.notes || {});
                      } catch (e) {
                        notes = {};
                      }
                      ...
                      const hasMetrics = notes.numberOfEmployees || notes.ipRegistrations || notes.gemPortalProducts;
                      const hasFunding = notes.loans || notes.angelFunding || notes.vcFunding;
                      
                      if (!hasProgressData && !hasMetrics && !hasFunding) return null;
                      
                      return (
                        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 border-2 border-blue-200 dark:border-blue-700 rounded-xl p-3 space-y-2">
                          <h4 className="text-xs font-bold text-blue-900 dark:text-blue-200 flex items-center space-x-2">
                            <BarChart3 className="w-4 h-4" />
                            <span>PROGRESS TRACKING</span>
                          </h4>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                            {notes.proofOfConcept && notes.proofOfConcept !== 'Not started' && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">PoC:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.proofOfConcept}</p>
                              </div>
                            )}
                            {notes.prototypeDevelopment && notes.prototypeDevelopment !== 'Not started' && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">Prototype:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.prototypeDevelopment}</p>
                              </div>
                            )}
                            {notes.productDevelopment && notes.productDevelopment !== 'Not started' && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">Product:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.productDevelopment}</p>
                              </div>
                            )}
                            {notes.fieldTrials && notes.fieldTrials !== 'Not started' && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">Field Trials:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.fieldTrials}</p>
                              </div>
                            )}
                            {notes.marketLaunch && notes.marketLaunch !== 'Not started' && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">Market:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.marketLaunch}</p>
                              </div>
                            )}
                            {notes.numberOfEmployees && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">Employees:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.numberOfEmployees}</p>
                              </div>
                            )}
                            {notes.ipRegistrations && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">IP Registrations:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.ipRegistrations}</p>
                              </div>
                            )}
                            {notes.gemPortalProducts && (
                              <div className="bg-white dark:bg-gray-700 p-2 rounded">
                                <span className="text-gray-600 dark:text-gray-400">GEM Products:</span>
                                <p className="font-semibold text-gray-900 dark:text-white text-xs">{notes.gemPortalProducts}</p>
                              </div>
                            )}
                          </div>
                          {(notes.loans || notes.angelFunding || notes.vcFunding) && (
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2 text-xs">
                              {notes.loans && (
                                <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded">
                                  <span className="text-gray-600 dark:text-gray-400">Loans:</span>
                                  <p className="font-semibold text-green-700 dark:text-green-300">₹{Number(notes.loans).toLocaleString()}</p>
                                </div>
                              )}
                              {notes.angelFunding && (
                                <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded">
                                  <span className="text-gray-600 dark:text-gray-400">Angel:</span>
                                  <p className="font-semibold text-green-700 dark:text-green-300">₹{Number(notes.angelFunding).toLocaleString()}</p>
                                </div>
                              )}
                              {notes.vcFunding && (
                                <div className="bg-green-50 dark:bg-green-900/20 p-2 rounded">
                                  <span className="text-gray-600 dark:text-gray-400">VC:</span>
                                  <p className="font-semibold text-green-700 dark:text-green-300">₹{Number(notes.vcFunding).toLocaleString()}</p>
                                </div>
                              )}
                            </div>
                          )}
                          {latestProgress.date && (
                            <p className="text-xs text-gray-500 dark:text-gray-400 text-right">
                              Updated: {new Date(latestProgress.date).toLocaleDateString()}
                            </p>
                          )}
                        </div>
                      );
