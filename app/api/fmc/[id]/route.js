...
export async function PUT(request, { params }) {
  try {
    ...
    const existingMeeting = await prisma.fMCMeeting.findUnique({
      where: { id },
      include: { startup: { select: { stage: true } } }
    });
    
    // Store the stage at which this FMC was completed in the agenda field
    ...
    const meeting = await prisma.fMCMeeting.update({
      where: { id },
      data: {
    ...
    console.error('Error updating FMC meeting:', error);
...
export async function DELETE(request, { params }) {
  try {
    ...
    const { id } = params;
    await prisma.fMCMeeting.delete({ where: { id } });
    return NextResponse.json({ message: 'FMC meeting deleted successfully' });
  } catch (error) {
    console.error('Error deleting FMC meeting:', error);
    return NextResponse.json({ message: 'Server error', error: error.message }, { status: 500 });
  }
}
